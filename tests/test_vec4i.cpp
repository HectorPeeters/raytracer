...
// i32 vec4i operations

TEST_CASE("i32 add vec4i", "[vec4i]") {
  vec4i a = vec4i(1, 2, 3, 4);
  vec4i res = 2 + a;
...
TEST_CASE("i32 div vec4i", "[vec4i]") {
  vec4i a = vec4i(1, 2, 3, 4);
  vec4i res = 2 / a;

  REQUIRE(res == vec4i(2, 1, 0, 0));
}
